...
* 공백을 포함한 태그나 카테고리를 입력하는 방법: 아래와 같이 메타정보를 입력한다.
        ...
        tags: 
          ...
          * VisualStudio
        ---
* 지킬에서 사이드메뉴 나 포스팅 내에 `ul` tag내에 있는 문자열로 목록을 필터링 하는 방법: [관련포스팅]({% post_url /blog/2015-04-25-깃허브-지킬에서-사이드메뉴-필터링 %})
...
*   code:
        ...
        여기서 부터 작성하면 리스트 아이템안에 코드블럭을 넣을 수 있게 됩니다.code
It’s worth noting that it’s possible to trigger an ordered list by accident, by writing something like this:
...
> 마크다운에서 코드블럭을 만들려면 앞에서 부터 4개의 공백문자를 넣거나 탭문자를 하나 넣으면 된다.
...
<pre class="prettyprint"><code>This is a code block.
</code></pre>
...
> 특별히 코드 하이라이팅 기능을 넣으려면 pre테그의 class속성에 "prettyprint"를 넣어 주면 된다.

<pre class="prettyprint"><code>tell application "Foo"
    beep
end tell
</code></pre>
...
> pre태그를 사용해서 코드블럭을 만들때 < 와 >를 사용하는 HTML, XML과 같이 마크다운 문법을 사용한는 코드인 경우 코드는 HTML 인코딩을 해 주어야 한다.

<pre class="prettyprint"><code>&lt;div class="footer"&gt;
    &amp;copy; 2004 Foo Corporation
&lt;/div&gt;
</code></pre>
...
> 아스테리스크를 문자로 직접 출력 하려면 앞에 역슬래쉬를 붙여주면 된다.
...
> 같은 이미지가 여려군데 들어갈 경우 `![이미지명][id]`와 같은 형식으로 이미지삽입 부분을 처리하고 아래쪽에 서 별도로 아래와 `[id]`에 대한 값을 지정해 준다.
